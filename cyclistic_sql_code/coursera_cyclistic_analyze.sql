...
#useful background/analysis: Different types of status within company. (NOTE: 4,331,704 total. Out of that total there is  2,799,949 member and 1,531,755 casual)
select member_casual,count(member_casual)as total_number_riders
from trip_data_2023
group by member_casual;

#Analysis: Total amount of riders for each bike types and rider status 
select member_casual, rideable_type, COUNT(*) as total_riders
from trip_data_2023
group by member_casual, rideable_type
order by member_casual, total_riders;

#analysis: number of bikes used per day
select CASE
...
#analysis: average length ride per day for each rider status
select CASE
    WHEN day_of_week = 1 THEN 'Mon' 
    WHEN day_of_week = 2 THEN 'Tues' 
    WHEN day_of_week = 3 THEN 'Wed'
