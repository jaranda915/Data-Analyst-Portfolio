...
#Analysis 1: progression of layoffs by using a rolling sum based on month and year (rolling total essentially just constantly adds the total_laid_off over each month)
...
#Analysis 2, step1: showing how many layoffs each company did per year
select company,year(`date`),sum(total_laid_off)
from layoffs_copy2
group by company,year(`date`)
order by 1 asc; 

#Analysis 2, step2: rank the amount of laid off employees per year (showing what company laid off the most employees in each year by getting top 5 most emplyoee lay off for that year)
...
#Analysis 3: shows layoff proportion. Shows which type of business got hit the hardest in proportion to their total employees (small companies got hit hardest, which is shown through "seed": start of business.)
